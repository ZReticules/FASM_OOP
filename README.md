# Полный набор всех создаваемых макросов и библиотек для FASM. Документация
## Содержание  
1. [FASM_OOP](#FASM_OOP)
   1. [Создание методов](#methods)
   2. [Наследование и полиморфизм](#polymorphism)
   3. [Вызов и процедур методов. Макрос @call](#@call)
   4. [Обработка аргументов макроса @call](#@call_args)
   5. [Обработчики методов](#meth_vectors)
...
### 1.2. Наследование и полиморфизм<a name="polymorphism"></a>  
...
### 1.3. Вызов методов и процедур. Макрос @call<a name="@call"></a>  
...
endp
```
### 1.4. Обработка аргументов макроса @call<a name="@call_args"></a>  
...
1. Строковые литералы 
   ...
   _**Важно! Так как автору не нравился порядок обработки аргументов на х64(по умолчанию fastcall заполняет аргументы слева направо) дефолтный обработчик аргументов oop_call отличается тем, что обрабатывет аргументы от последнего к первому. Таким образом указатель на обьект спокойно можно хранить в rcx и не опасаться, что при передаче параметров он будет затерт раньше времени**_  
2. Литералы с типом значения  
...
### 1.5. Обработчики методов<a name="meth_vectors"></a>  
...
#### 1.5.1. Виртуальные обьекты<a name="virt_obj"></a>  
...
#### 1.5.2. Инлайн методы: специальные возможности<a name="inline"></a>  
В макросы-обработичики инлайн методов параметры передаются так, как если бы они передавались и в статические/виртуальные методы. Однако если в самом методе нам надо выполнить какие-то операции над обьектом, у нас могут возникнуть сложности с тем, чтобы получить значение поля обьекта или переместить параметр в регистр. Для решения этих проблем были созданы макросы fillParam и inlineObj. Так же, как уже было сказано ранее, при передаче аргументов, если в начале указать ?, аргумент будет передан в необработанном виде. Так же для таких методов можно получить количество аргументов в виде переменной этапа компиляции `__argscount__`.  
...
```
### 1.6. @jret (для x64)<a name="@jret"></a>  
Данный макрос используется для оптимизации выхода из процедур на х64. Он позволяет прыгнуть на процедуру(или метод) вместо ret с передаче параметров через регистры.  
При использовании подменяет последний вызов call на прыжок в функцию(для инлайн методов, для обычных методов и функций просто прыжок).  
Из этого следует ограничение - не более 4-х параметров вызываемой функции. Экспериментальная функция, использовать с осторожностью!  
### 1.7. Константные поля структур<a name="const"></a>  
...
## 2. DIALOGFORM<a name="dialogform"></a>  
